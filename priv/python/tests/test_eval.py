...
from lux.eval import decode_term, encode_term, execute
...
def test_encode_collections():
    ...
    # Nested collections
    assert encode_term({"list": [1, 2], "map": {"x": 10}}) == {
        b"list": [1, 2],
        b"map": {b"x": 10},
    }
...
def test_encode_struct_conversion():
    ...
    # Basic struct
    result = encode_term({"__class__": "user", "name": "Alice", "role": "admin"})
    ...
    # Nested module name
    result = encode_term({"__class__": "data.types.point", "x": 1, "y": 2})
...
def test_execute_with_variables():
    ...
    variables = {b"data": {b"numbers": [1, 2, 3], b"name": b"test"}}
