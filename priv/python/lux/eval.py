...
def encode_term(term):
    ...
    if term is None:
        ...
        return term.encode(
            "utf-8"
        )  # Convert strings to binary for proper Elixir string handling
...
def execute(code, variables=None):
    ...
    try:
        ...
        if len(tree.body) > 1:
            ...
            exec(
                ast.unparse(ast.Module(body=tree.body[:-1], type_ignores=[])),
                globals_dict,
            )
...
def check_package(package_name):
    ...
    return encode_term(
        {"available": version is not None, "version": version if version else None}
    )
...
def import_package(package_name):
    ...
    return encode_term({"success": success, "error": error if error else None})
