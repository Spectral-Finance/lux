defmodule Lux.Lenses.Etherscan.TokenNftContractTx do
  @moduledoc """
  Lens for fetching ERC-721 (NFT) token transfer events for a specific contract from the Etherscan API.

  ## Examples

  ```elixir
  # Get ERC-721 transfers for a token contract
  Lux.Lenses.Etherscan.TokenNftContractTx.focus(%{
  ...
  use Lux.Lens,
    ...
    schema: %{
      ...
      properties: %{
        ...
        contractaddress: %{
          type: :string,
          description: "NFT contract address to query transfers for (must be valid hex format)",
